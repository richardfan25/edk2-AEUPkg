...
// ECGZ : (Board=ECGF f/w) ====> (ECGS f/w image) ====> (Board=ECGS f/w)

// Key  : EC RAM [FCh] = 0x81 (need to re-build firmware for EC_RAM[FCh]=0x81 for using ecgz code base)
...
#include "ecgz.h"
/*==============================================================*/
const chip_t ecgz_chips ={
	RDC_VID,							/* vid */
	RDC_PID_EIO_IS200,					/* pid */
};

const codebase_t ecgz_cb = {
	sizeof(ecgz_chips)/sizeof(chip_t),	/* chip_num */
	&ecgz_chips,						/* chip_list */
	ECGZW_CB_ID,						/* cbid */
	{									/* func */
		&ecgz_ec_init,						/* init */
		&ecgz_ec_exit,						/* exit */
		&ecgz_get_embedic_info,				/* get_embedic_info */
		&ecgz_get_bin_info,					/* get_binary_info */
		&ecgz_ec_update,					/* update_firmware */
		&ecgz_ec_check_model_name,			/* check_model_name */
	},
};
...
//		   -1: fail
/*==============================================================*/
int ecgz_check_bin_sum(binary_t *bin)
{
    int ret = 0;
    uint8_t *buf = NULL;
    uint32_t offset;
...
/*==============================================================*/
int ecgz_check_header(ecgz_rom_header_t *bHeader)
{
	if((memcmp(bHeader->signature, ECGZ_ROM_SIGN, 4) != 0) ||
		(bHeader->length != sizeof(ecgz_rom_header_t)) ||
		(bHeader->magic_num != ECGZ_ROM_MAGIC)){
		return -1;
	}
	return 0;
}
...
// Output:  None
/*==============================================================*/
void ecgz_print_header(ecgz_rom_header_t *bHeader)
{
...
/*==============================================================*/
ecgz_rom_header_t *ecgz_get_header(binary_t *bin)
{
    uint32_t 	readcount;
	ecgz_rom_header_t *header = (ecgz_rom_header_t *)malloc(sizeof(ecgz_rom_header_t));
	...
	// Get Sign
    fseek(bin->ctrl, 0, SEEK_SET);
	readcount = (uint32_t)fread((uint8_t *)header, 1, sizeof(ecgz_rom_header_t), bin->ctrl);
	
    if((readcount != sizeof(ecgz_rom_header_t)) ||
		(ecgz_check_header(header) != 0)){
...
// Note: must be free the header point if it is not needed
/*==============================================================*/
ecgz_rom_header_t *ecgz_get_bootheader(binary_t *bin)
{
    uint32_t 	readcount;
	ecgz_rom_header_t *header = ecgz_get_header(bin);
...
int ecgz_get_bin_info(binary_t *bin)
{
    ...
    ecgz_rom_header_t *bHeader = ecgz_get_header(bin);
	
	if(ecgz_check_bin_sum(bin) != 0)
		goto exit;
...
/*==============================================================*/
static int ecgz_isp_erase_option(binary_t *bin)
{
	uint32_t addr;
	uint32_t start;
	ecgz_rom_header_t *h = ecgz_get_header(bin);
	if(h == NULL)
		return -1;
...
/*==============================================================*/
static int ecgz_isp_update_boot(binary_t *bin)
{
	int 		rtn = -1;
	uint8_t 	*buf = NULL;
	uint8_t 	*vbuf = NULL;
...
/*==============================================================*/
static int ecgz_isp_update_app(binary_t *bin)
{
	uint8_t *buf = NULL;
	uint8_t *vbuf = NULL;
	uint32_t offset;
	...
	fseek(bin->ctrl, start + ECGZ_HEADER_SIZE, SEEK_SET);
	max = h->size + FLASH_PAGE_SIZE;
	for(offset = ECGZ_HEADER_SIZE; offset < max ; offset += FLASH_PAGE_SIZE){
...
int ecgz_ec_update(binary_t *bin, isp_flag_t upflag)
{
	...
	// erase option
	if(ecgz_isp_erase_option(bin) != 0){
		fprintf(stderr, "ERROR: Failed to erase option.\n");
		goto exit;
	}

	// update boot area
	if(ecgz_isp_update_boot(bin) != 0){
		fprintf(stderr, "ERROR: Failed to update boot.\n");
		goto exit;
	}
	
	// update app area
	if(ecgz_isp_update_app(bin) != 0){
